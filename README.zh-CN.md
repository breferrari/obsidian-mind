...
**会话得到的能力：** `search`（语义 + 关键词）、`expand`（笔记的出链与反链）、`recall`（限定到该仓库的持久经验）、`remember`（记录一条经验）、`record_work`（归档所做的工作）、`health`（接线是否完好）。此外还把笔记作为可读资源暴露出来。
...
**默认只开放 `brain/` 和 `reference/`。** 你的工作笔记、人物笔记、1:1 记录和评估材料在你明确开放之前都留在围栏之内。开放范围刻意**不**跟随 `user_content_roots` — 那个键的含义是"这是我的内容，升级时请保留"，与"这可以离开 vault"是两个不同的问题。放宽范围要么是显式的，要么不会发生。无论配置如何，记忆永远不会作为普通笔记提供；每次读取都会连同调用方仓库一起记入日志。

> [!WARNING]
> **切勿在项目仓库中注册原始 QMD 服务器。** `.mcp.json` 里的一行就会让该会话获得对整个 vault 的无围栏语义搜索 — 包括每一份客户笔记和所有私密内容。`om` 服务器的存在，正是为了站在索引前面决定什么可以出去。
