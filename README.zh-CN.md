...
**围栏。** 关注点是外泄 — vault 内容出现在提交信息、PR 正文或公开 issue 中 — 而不是对你隐藏文件。只有当笔记所在文件夹被开放、文件名不在 `mcp_never_expose` 中、且未标记 `private` 时，它才会被提供。两个列表都在 `vault-manifest.json` 中配置；默认值刻意收得很窄。无论配置如何，记忆永远不会作为普通笔记提供。每次读取都会连同调用方仓库一起记入日志。
