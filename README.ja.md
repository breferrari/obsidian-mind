...
**フェンス。** 懸念はエグレス — ボールトの内容がコミット、PR本文、公開Issueに紛れ込むこと — であり、あなたからファイルを隠すことではありません。ノートが提供されるのは、そのフォルダが公開対象で、ファイル名が`mcp_never_expose`になく、`private`タグが付いていない場合のみです。両方のリストは`vault-manifest.json`で設定します。デフォルトは意図的に狭くしてあります。メモリは設定内容にかかわらず、通常のノートとしては決して提供されません。すべての読み取りは呼び出し元リポジトリとともに記録されます。
