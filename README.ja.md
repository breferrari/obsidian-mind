...
**セッションが得られるもの:** `search`（セマンティック＋キーワード）、`expand`（ノートのリンクと被リンク）、`recall`（そのリポジトリにスコープされた恒久的な教訓）、`remember`（教訓の記録）、`record_work`（作業内容の記録）、`health`（配線は正常か）。加えて、ノートを読み取り可能なリソースとして提供します。
...
**デフォルトは`brain/`と`reference/`のみです。** 作業ノート、人物ノート、1on1、評価資料は、あなたが明示的に許可するまでフェンスの内側に留まります。公開範囲は意図的に`user_content_roots`に追従**しません** — あのキーは「これは自分のコンテンツなのでアップグレード時に保持せよ」という意味であり、「これはボールトの外に出してよい」とは別の問いだからです。範囲を広げるのは明示的な操作であり、それ以外では起こりません。メモリは設定内容にかかわらず通常のノートとしては決して提供されず、すべての読み取りは呼び出し元リポジトリとともに記録されます。デフォルトは意図的に狭くしてあります。メモリは設定内容にかかわらず、通常のノートとしては決して提供されません。すべての読み取りは呼び出し元リポジトリとともに記録されます。

> [!WARNING]
> **生のQMDサーバーをプロジェクトリポジトリに登録しないでください。** `.mcp.json`の一行が、そのセッションにボールト全体への無制限なセマンティック検索を与えます — すべてのクライアントノート、すべての非公開情報を含めて。`om`サーバーは、インデックスの前に立って何を外に出すかを判断するために存在します。
