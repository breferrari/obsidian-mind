...
| 階層 | 内容 | タイミング | コスト |
|------|------|------------|--------|
| **常時** | `CLAUDE.md` + SessionStartメタデータ（ファイル名、git概要、タスク） | セッション開始時 | ~2Kトークン |
...
SessionStartは**メタデータとファイル名のみ**を読み込みます。起動時にファイル全体は読み込みません。ClaudeはQMDで意味検索してからファイルを読むため、関連する情報のみを取得します。分類フックはメッセージごとに軽量なPython呼び出し1回です。検証フックはMarkdown書き込み時のみ発火し、ボールト外のファイルはスキップします。
