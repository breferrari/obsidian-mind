...
**세션이 얻는 것:** `search`(시맨틱 + 키워드), `expand`(노트의 링크와 백링크), `recall`(해당 저장소로 스코프된 지속적 교훈), `remember`(교훈 기록), `record_work`(작업 내용 기록), `health`(배선이 정상인지). 여기에 노트를 읽을 수 있는 리소스로 제공합니다.
...
**기본값은 `brain/`과 `reference/` 뿐입니다.** 작업 노트, 인물 노트, 1:1, 평가 근거는 당신이 달리 지정하기 전까지 울타리 안에 남습니다. 노출 범위는 의도적으로 `user_content_roots`를 따르지 **않습니다** — 그 키는 "이것은 내 콘텐츠이니 업그레이드 시 보존하라"는 뜻이며, "이것은 볼트 밖으로 나가도 된다"와는 다른 질문이기 때문입니다. 범위를 넓히는 것은 명시적이거나 아예 일어나지 않습니다. 메모리는 설정과 무관하게 일반 노트로 제공되지 않으며, 모든 읽기는 호출한 저장소와 함께 기록됩니다. 기본값은 의도적으로 좁게 잡혀 있습니다. 메모리는 설정과 무관하게 일반 노트로는 결코 제공되지 않습니다. 모든 읽기는 호출한 저장소와 함께 기록됩니다.

> [!WARNING]
> **원시 QMD 서버를 프로젝트 저장소에 등록하지 마세요.** `.mcp.json`의 한 줄이 그 세션에 볼트 전체에 대한 제한 없는 시맨틱 검색 권한을 줍니다 — 모든 클라이언트 노트와 모든 비공개 내용까지. `om` 서버는 인덱스 앞에 서서 무엇이 나갈지 결정하기 위해 존재합니다.
