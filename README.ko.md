...
**생각하는 것은 `reason` 하나입니다.** 나머지가 검색이라면, 이것은 두 번째 Claude 세션으로 볼트를 읽고 노트를 가로지르는 판단이 필요한 질문에 답합니다 — *지금 하려는 일이 이 여섯 개의 노트가 결정한 바와 일치하는가?* 스스로 검색으로 씨앗을 얻으므로 먼저 검색할 필요가 없습니다. 사용자 본인의 CLI 기본 모델로 동작하므로, 볼트는 지금 작업 중인 수준과 같은 수준으로 답합니다. 그리고 어떤 상한도 없습니다 — 당신의 기기에서, 당신의 인증으로 도는 Claude이기 때문입니다. 호출마다 비용·턴 수·모델·실제 소요 시간이 기록됩니다. 답변에는 `confidence: inferred`가 붙고 그 자체로 메모리에 기록되지 않습니다 — 남길 가치가 있는지는 당신의 세션이 정합니다.
...
**무엇을 제공하는가.** 당신의 볼트, 당신의 노트, 당신의 세션입니다. 기본값은 볼트가 이미 자신의 콘텐츠로 선언한 것(`user_content_roots`)을, 작성한 그대로의 단위로 사용합니다(`work/` 전체가 아니라 `work/active/`). `vault-manifest.json`의 `mcp_exposed_roots`를 설정하는 것은 이 볼트가 **당신이 공유할 권한이 없는** 자료 — 고용주의 기밀 정보나 고객의 데이터 — 를 담고 있을 때뿐입니다. `private` 태그가 붙은 노트는 제공되지 않으며, 메모리는 자체 스코프를 갖기 때문에 일반 노트로 제공되지 않습니다. 모든 읽기는 호출한 저장소와 함께 기록됩니다.
