...
| 계층 | 내용 | 시점 | 비용 |
|------|------|------|------|
| **항상** | `CLAUDE.md` + SessionStart 메타데이터 (파일명, git 요약, 작업) | 세션 시작 시 | ~2K 토큰 |
...
SessionStart는 **메타데이터와 파일명만** 로드합니다 — 시작 시 전체 파일을 읽지 않습니다. Claude는 QMD를 통해 의미 기반 검색 후 파일을 읽으므로 관련 정보만 가져옵니다. 분류 훅은 메시지당 가벼운 Python 호출 1회입니다. 검증 훅은 마크다운 쓰기 시에만 실행되며 볼트 외부 파일은 건너뜁니다.
