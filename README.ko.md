...
**울타리.** 관심사는 유출입니다 — 볼트 자료가 커밋, PR 본문, 공개 이슈에 들어가는 것 — 당신에게서 파일을 숨기는 것이 아닙니다. 노트는 폴더가 공개 대상이고, 파일명이 `mcp_never_expose`에 없으며, `private` 태그가 없을 때만 제공됩니다. 두 목록 모두 `vault-manifest.json`에서 설정합니다. 기본값은 의도적으로 좁게 잡혀 있습니다. 메모리는 설정과 무관하게 일반 노트로는 결코 제공되지 않습니다. 모든 읽기는 호출한 저장소와 함께 기록됩니다.
